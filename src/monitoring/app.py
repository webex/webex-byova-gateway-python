...
def api_debug_sessions():
    ...
    try:
        debug_info = {
            ...
            'has_active_sessions_attr': hasattr(gateway_server_instance, 'active_sessions') if gateway_server_instance else False,
            'has_get_active_sessions': hasattr(gateway_server_instance, 'get_active_sessions') if gateway_server_instance else False,
            'has_get_connection_events': hasattr(gateway_server_instance, 'get_connection_events') if gateway_server_instance else False,
        }
        
        if gateway_server_instance:
            if hasattr(gateway_server_instance, 'active_sessions'):
                debug_info['active_sessions_count'] = len(gateway_server_instance.active_sessions)
                debug_info['active_sessions_keys'] = list(gateway_server_instance.active_sessions.keys())
            
            if hasattr(gateway_server_instance, 'get_connection_events'):
                debug_info['connection_events_count'] = len(gateway_server_instance.get_connection_events())
...
def api_test_create_session():
    ...
    try:
        if gateway_server_instance:
            ...
            gateway_server_instance.add_connection_event('start', test_session_id, test_agent_id, 
                                                      customer_org_id='test-customer-org')
    ...
    if gateway_server_instance:
        try:
            ...
            if hasattr(gateway_server_instance, 'get_active_sessions'):
                ...
                for session_id, session_data in active_sessions_data.items():
                    active_sessions.append({
                        ...
                        'customer_org_id': session_data.get('customer_org_id', 'Unknown'),
            ...
            # Fallback to direct access if method doesn't exist
                ...
                for session_id, session_data in gateway_server_instance.active_sessions.items():
                    active_sessions.append({
                        'session_id': session_id,
                        'agent_id': session_data.get('agent_id', 'Unknown'),
                        'customer_org_id': session_data.get('customer_org_id', 'Unknown'),
...
def run_web_app(router_instance_param: 'VirtualAgentRouter', 
